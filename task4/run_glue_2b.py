# coding=utf-8
# Task 4 (2b): all_reduce with torch.profiler — profiles 3 training steps, skips first.
#
# Run on each node:
#   python run_glue_2b.py [args] --master_ip <ip> --master_port <port> \
#       --world_size 4 --local_rank <0|1|2|3>
#
# After training, open trace_rank<N>_all_reduce.json in chrome://tracing
...
def sync_gradients_all_reduce(model, args):
    """Average gradients using all_reduce (sum then divide by world_size)."""
...
def train(args, train_dataset, model, tokenizer):
    """Train with all_reduce and torch.profiler (skip step 0, record steps 1-3)."""
    ...
    trace_file = f"trace_rank{args.local_rank}_all_reduce.json"
