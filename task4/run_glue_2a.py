# coding=utf-8
# Task 4 (2a): gather/scatter with torch.profiler — profiles 3 training steps, skips first.
#
# Run on each node:
#   python run_glue_2a.py [args] --master_ip <ip> --master_port <port> \
#       --world_size 4 --local_rank <0|1|2|3>
#
# After training, open trace_rank<N>_gather_scatter.json in chrome://tracing
...
def sync_gradients_gather_scatter(model, args):
    """Average gradients across all workers using gather then scatter."""
...
def train(args, train_dataset, model, tokenizer):
    """Train with gather/scatter and torch.profiler (skip step 0, record steps 1-3)."""
    ...
    trace_file = f"trace_rank{args.local_rank}_gather_scatter.json"

    # schedule: wait=1 (skip step 0), warmup=0, active=3 (record steps 1-3), repeat=1
    ...
    with torch.profiler.profile(
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch}",
                ...
                # Stop after recording 4 steps (wait=1 + active=3)
                if step >= 3:
                    profiling_done = True
                    break
            if profiling_done:
    ...
    if args.local_rank in [-1, 0]:
        logger.info("Chrome trace saved to %s", trace_file)
...
def main():
    ...
    if os.path.exists(args.output_dir) and os.listdir(args.output_dir) and args.do_train and not args.overwrite_output_dir:
        raise ValueError("Output directory already exists. Use --overwrite_output_dir.")
