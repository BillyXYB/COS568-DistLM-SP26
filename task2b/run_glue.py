# coding=utf-8
# Task 2(b): Distributed data-parallel training with all_reduce gradient synchronization.
#
# After each backward pass, all_reduce sums gradients across all workers and
# each worker divides by world_size to get the average.
#
# Run on each node:
...
def sync_gradients_all_reduce(model, args):
    """Average gradients across all workers using all_reduce (sum then divide)."""
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
        logger.info("***** Running training (all_reduce) *****")
