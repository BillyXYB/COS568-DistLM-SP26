# coding=utf-8
# Task 3: Distributed data-parallel training using PyTorch DistributedDataParallel (DDP).
#
# DDP automatically handles gradient synchronization via all-reduce hooks registered
# on each parameter during the backward pass, overlapping communication with computation.
#
# Run on each node:
...
def train(args, train_dataset, model, tokenizer):
    """Train the model using DistributedDataParallel (no manual gradient sync)."""
    ...
    if args.local_rank in [-1, 0]:
        logger.info("***** Running training (DDP) *****")
